  ...
  for (int i = 0; i < count; i++) { //scan cases
   ...
   //  if(cname.contains("orrelation")){
    for (int k = 0; k < stepcount; k++) { //scan steps
      def stpname = testRunner.testCase.testSuite.project.getTestSuiteAt(j).getTestCaseAt(i).getTestStepAt(k).getName()
...
{
   //log.info testRunner.testCase.testSuite.project.getTestSuiteAt(j).getTestCaseAt(i).getTestStepAt(k).getAssertionAt(0)
   testStep.removeAssertion( testStep.getAssertionByName( "Response SLA") )
   log.info stpname
